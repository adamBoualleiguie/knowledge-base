---
title: "Debugging playbook: the failures that ate my evenings (and the fixes)"
description: "Repeat CI failures from a polyglot bazel_ci job—toolchains, tags, lockfiles, sandbox—and the order that usually fixes them fastest."
...
# Debugging playbook: the failures that ate my evenings (and the fixes)
...
<Terminal
  ...
  commands={[
    {
      command: "bazelisk build //path:target -s        # see subcommands",
      output: "",
    },
    {
      command: "bazelisk test //path:test --test_output=all",
      output: "",
    },
    {
      command: "bazelisk query --output=build //path:target",
      output: "",
    },
    {
      command: "bazelisk query 'deps(//path:target)' --output=graph > /tmp/graph.dot",
      output: "",
    },
  ]}
/>
