---
title: "Remote cache: `.bazelrc.user`, forking ethics, and speed that matters"
description: "Disk cache in Actions vs optional remote cache via secrets—speed without making forks depend on your infrastructure."
...
# Remote cache: `.bazelrc.user`, forking ethics, and speed that matters
...
<Terminal
  ...
  commands={[
    ...
    {
      command: "--remote_cache=https://cache.example.com \\",
      output: "",
    },
    {
      command: "--remote_cache_header=\"Authorization=Bearer $TOKEN\"",
      output: "",
    },
  ]}
/>
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong><code>~/.cache/bazel</code></strong> on Actions</td>
      <td><strong>Same</strong> runner or repeated runs see warm <strong>local</strong> blobs</td>
    </tr>
    <tr>
      <td><strong>Remote cache</strong></td>
      <td><strong>Different</strong> runners (or teammates) share <strong>action outputs</strong></td>
    </tr>
    <tr>
      <td><strong><code>MODULE.bazel.lock</code></strong></td>
      <td><strong>Dependency graph</strong> is reproducible — cache is an optimization, not the contract</td>
    </tr>
  </tbody>
</table>
...
<Terminal
  ...
  commands={[
    {
      command: "bazelisk build //src/checkout:checkout_image --config=ci \\",
      output: "# Any target \u2014 add remote flags as needed",
    },
    {
      command: "--remote_cache=https://cache.example.com \\",
      output: "",
