---
title: "Rust `shipping`: `rules_rust`, `crate_universe`, dynamic linking, and distroless **cc**"
description: "`rules_rust` and `crate_universe` for the shipping service—dynamic linking, glibc vs static myths, tests, and OCI on distroless cc to match the Dockerfile binary."
...
# Rust `shipping`: `rules_rust`, `crate_universe`, dynamic linking, and distroless **cc**
...
<Terminal
  ...
  commands={[
    {
      command: "CARGO_BAZEL_REPIN=1 bazelisk sync --only=shipping_crates",
      output: "",
    },
  ]}
/>
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>Unknown crate / label after edit</strong></td>
      <td><strong><code>CARGO_BAZEL_REPIN</code></strong> + <strong><code>sync --only=shipping_crates</code></strong>; commit <strong><code>cargo-bazel-lock.json</code></strong>.</td>
    </tr>
    <tr>
      <td><strong>Binary runs locally, dies in container</strong></td>
      <td><strong>Wrong OCI base</strong> — <strong>cc</strong> vs <strong>static</strong>; <strong><code>ldd</code></strong>-style thinking.</td>
    </tr>
    <tr>
      <td><strong>Tests fail only in CI</strong></td>
      <td><strong>Rust toolchain</strong> version on runner vs laptop; <strong><code>rust_test</code></strong> <strong><code>deps</code></strong> include <strong><code>normal_dev</code></strong>.</td>
    </tr>
    <tr>
      <td><strong>Slow / huge analysis</strong></td>
      <td><strong><code>src/shipping/target</code></strong> still ignored?</td>
    </tr>
  </tbody>
</table>
