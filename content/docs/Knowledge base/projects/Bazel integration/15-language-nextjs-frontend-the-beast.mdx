---
title: "Next.js `frontend`: the beast (build, lint, and OCI)"
description: "Next.js under Bazel—standalone output, worker shims, lint, `no-sandbox` honesty, and OCI layouts that still match what Compose expects at runtime."
...
# Next.js `frontend`: the beast (build, lint, and OCI)
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>Duplicate React / styled-components</strong></td>
      <td><strong><code>public_hoist_packages</code></strong>; worker shim loaded; <strong>webpack</strong> path under Bazel.</td>
    </tr>
    <tr>
      <td><strong>Sandbox / symlink errors on <code>next_build</code></strong></td>
      <td><strong><code>no-sandbox</code></strong> still on <strong><code>next_build</code></strong>? (If you “fix” it, prove <strong>standalone</strong> has no illegal links.)</td>
    </tr>
    <tr>
      <td><strong>Broken preload / weird Node startup</strong></td>
      <td>Did I add <strong><code>--require</code></strong> to <strong><code>NODE_OPTIONS</code></strong> on <strong><code>js_run_binary</code></strong>? <strong>Don’t.</strong></td>
    </tr>
    <tr>
      <td><strong>Cypress downloading during analysis</strong></td>
      <td><strong><code>lifecycle_hooks_exclude</code></strong> on <strong><code>npm_frontend</code></strong>.</td>
    </tr>
    <tr>
      <td><strong><code>.env</code></strong> <strong>surprises</strong></td>
      <td>Next reads <strong><code>.env</code></strong> via <strong><code>next.config.js</code></strong> paths — <strong>hermetic CI</strong> may see <strong>empty</strong> values unless I thread <strong>env</strong> into actions on purpose.</td>
    </tr>
  </tbody>
</table>
