---
title: "Ruby `email`: `rules_ruby`, `bundle_fetch`, glibc vs Alpine, and a gems smoke test"
description: "`rules_ruby`, vendored bundles, libc policy (Debian-slim Bazel vs Alpine Docker), native gems, and smoke tests without pretending the two worlds are identical."
...
# Ruby `email`: `rules_ruby`, `bundle_fetch`, glibc vs Alpine, and a gems smoke test
...
<table>
  <thead>
    <tr>
      <th>Aspect</th>
      <th><strong>Dockerfile (Alpine)</strong></th>
      <th><strong>Bazel <code>email_image</code></strong></th>
    </tr>
  </thead>
  <tbody>
    <tr>
      ...
      <td><strong>glibc</strong> (<strong>bookworm</strong> slim)</td>
    </tr>
    <tr>
      <td><strong>Native gems</strong></td>
      <td>Built against <strong>musl</strong> toolchain in image</td>
      <td><strong><code>bundle_fetch</code></strong> under <strong>portable MRI</strong> / <strong>glibc</strong></td>
    </tr>
    <tr>
      <td><strong>When I use it</strong></td>
      <td><strong>Published</strong> multi-arch matrix, <strong>Compose</strong></td>
      <td><strong><code>bazel build</code></strong>, <strong><code>docker load</code></strong>, <strong><code>otel/demo-email:bazel</code></strong></td>
    </tr>
  </tbody>
</table>
...
<Terminal
  ...
  commands={[
    {
      command: "bazelisk build //src/email:email --config=ci",
      output: "",
    },
    {
      command: "bazelisk test  //src/email:email_gems_smoke_test --config=ci --config=unit",
      output: "",
    },
    {
      command: "bazelisk build //src/email:email_image --config=ci",
      output: "",
    },
    {
      command: "bazelisk run  //src/email:email_load",
      output: "",
    },
    {
      command: "docker image ls | grep demo-email",
      output: "",
    },
  ]}
/>
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>Bundle / gem resolution errors</strong></td>
      <td><strong><code>Gemfile.lock</code></strong> <strong>PLATFORMS</strong>; <strong><code>force_ruby_platform</code></strong> on <strong>protobuf</strong>; <strong>Ruby 3.4.8</strong> matches <strong><code>.ruby-version</code></strong>.</td>
    </tr>
    <tr>
      <td><strong><code>rb_test</code></strong> hangs</td>
      <td>Accidental <strong><code>require "sinatra"</code></strong> <strong>classic</strong> — use <strong><code>sinatra/base</code></strong> only in smoke.</td>
    </tr>
    <tr>
      <td><strong>Native extension mismatch</strong></td>
      <td><strong>Alpine</strong> vs <strong>slim</strong> — am I mixing <strong>musl</strong> binaries into <strong>glibc</strong> image?</td>
    </tr>
    <tr>
      <td><strong>Missing <code>views/</code></strong></td>
      <td><strong><code>rb_library</code></strong> <code>data = glob(["views/**"])</code>; <strong><code>email_app_layer</code></strong> includes <strong>views</strong>.</td>
    </tr>
  </tbody>
</table>
