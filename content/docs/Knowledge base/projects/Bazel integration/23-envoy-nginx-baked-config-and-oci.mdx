---
title: "Envoy + nginx: baked config, `envsubst`, and “config images” in Bazel"
description: "`frontend-proxy` and `image-provider` as config-first services—template bake at build time, `oci_image`, `sh_test` smoke, and how that differs from Dockerfile `envsubst` at container start."
...
# Envoy + nginx: baked config, `envsubst`, and “config images” in Bazel
...
<table>
  <thead>
    <tr>
      ...
      <th><strong>Bazel <code>oci_image</code></strong></th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><strong>When config is rendered</strong></td>
      <td><strong>Container start</strong></td>
      <td><strong><code>bazel build</code></strong> (unless you wrap bake in a <strong>custom</strong> env)</td>
    </tr>
    <tr>
      <td><strong>Overriding upstreams</strong></td>
      <td>Change <strong>env</strong> at <strong><code>docker run</code></strong></td>
      <td><strong>Rebuild</strong> with env for <strong><code>genrule</code></strong>, or use <strong>Docker</strong> path</td>
    </tr>
    <tr>
      <td><strong>Debug</strong></td>
      <td><strong>nginx</strong> <strong><code>cat</code></strong>’s config at start</td>
      <td><strong>Baked</strong> file is in <strong>layer</strong> — inspect tar or <strong>run</strong> container</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong><code>envsubst: command not found</code></strong></td>
      <td><strong><code>gettext-base</code></strong> installed; <strong>PATH</strong> in CI.</td>
    </tr>
    <tr>
      <td><strong><code>genrule</code></strong> fails in sandbox</td>
      <td><strong><code>no-sandbox</code></strong> still on <strong><code>genrule</code></strong>; host tool access.</td>
    </tr>
    <tr>
      <td><strong>Wrong upstream in image</strong></td>
      <td><strong>Defaults</strong> in <strong><code>{'bake_*.sh'}</code></strong> vs <strong>Compose</strong>; <strong>rebuild</strong> after template edits.</td>
    </tr>
    <tr>
      <td><strong>nginx user / permission</strong></td>
      <td><strong><code>user = "101"</code></strong> on <strong><code>image_provider_image</code></strong> matches <strong>unprivileged</strong> image.</td>
    </tr>
  </tbody>
</table>
