---
title: "JVM: `ad` (Java) and `fraud-detection` (Kotlin), gRPC, and deploy-JAR OCI"
description: "Java and Kotlin microservices in one Bazel graph—Maven pins, gRPC codegen, deploy JARs, OTel agent parity, and OCI images next to Gradle-shaped Dockerfiles."
...
# JVM: `ad` (Java) and `fraud-detection` (Kotlin), gRPC, and deploy-JAR OCI
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>Missing symbol from proto / gRPC</strong></td>
      <td><strong><code>//pb:demo_java_grpc</code></strong> on <strong><code>deps</code></strong>; proto changes need graph rebuild.</td>
    </tr>
    <tr>
      <td><strong>Duplicate classes / Maven resolution</strong></td>
      <td>New coordinate <strong>duplicates</strong> grpc-java/protobuf — <strong><code>known_contributing_modules</code></strong>, avoid re-listing Guava.</td>
    </tr>
    <tr>
      <td><strong>Kotlin main not found</strong></td>
      <td><strong><code>main_class</code></strong> for <strong><code>MainKt</code></strong>; <strong><code>kt_jvm_library</code></strong> linked via <strong><code>runtime_deps</code></strong>.</td>
    </tr>
    <tr>
      <td><strong>Native / codegen errors</strong></td>
      <td><strong><code>.bazelrc</code></strong> <strong><code>-std=c++17</code></strong> for host C++.</td>
    </tr>
    <tr>
      <td><strong>“Works in Docker, not in Bazel image”</strong></td>
      <td>Agent / <strong><code>JAVA_TOOL_OPTIONS</code></strong> differences; not claiming parity until I add that layer.</td>
    </tr>
  </tbody>
</table>
