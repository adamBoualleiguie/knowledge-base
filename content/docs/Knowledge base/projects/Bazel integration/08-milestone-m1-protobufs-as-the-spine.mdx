---
title: "Milestone M1: protobufs as the spine (`//pb` + policy)"
description: "Shared `demo.proto`, `//pb:*` targets, CI checks, and how Bazel and the legacy Docker generate path coexist while protos stay the contract."
...
# Milestone M1: protobufs as the spine (`//pb` + policy)
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong><code>pb/demo.proto</code></strong></td>
      <td>The <strong>contract</strong> everyone shares.</td>
    </tr>
    <tr>
      <td><strong>Docker + <code>make clean docker-generate-protobuf</code></strong></td>
      <td>Still the path that produces <strong>committed</strong> files under <code>{'src/*'}</code> (Go <code>genproto/</code>, Python <code>{'demo_pb2*.py'}</code>, TS, etc.). CI <strong><code>protobufcheck</code></strong> insists the tree is <strong>clean</strong> after regeneration.</td>
    </tr>
    <tr>
      <td><strong>Bazel <code>{'//pb:*'}</code></strong></td>
      <td><strong>Parallel proof</strong> the same <code>.proto</code> builds under the Bazel graph — first <strong>descriptor / Go gRPC</strong>, later more languages as the graph grew.</td>
    </tr>
  </tbody>
</table>
...
<Terminal
  ...
  commands={[
    {
      command: "bazelisk build //pb:demo_proto --config=ci",
      output: "# Descriptor + proto root",
    },
    {
      command: "bazelisk build //pb:go_grpc_protos --config=ci",
      output: "# Both Go gRPC codegen targets (M1 bundle)",
    },
    {
      command: "bazelisk build //pb:java_grpc_protos --config=ci",
      output: "# Java bundle (as wired in current pb/BUILD.bazel)",
    },
  ]}
/>
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>BZ-030</strong></td>
      <td><code>proto_library</code> for <code>demo.proto</code></td>
    </tr>
    <tr>
      <td><strong>BZ-031</strong></td>
      <td>Go <code>go_proto_library</code> + gRPC for both import paths</td>
    </tr>
    <tr>
      <td><strong>BZ-037</strong></td>
      <td>Written <strong>policy</strong> for canonical proto vs committed gen vs Bazel proof</td>
    </tr>
    <tr>
      <td><strong>BZ-038</strong></td>
      <td>CI <strong>dual-run</strong> protobuf gate</td>
    </tr>
    <tr>
      <td><strong>BZ-032–036</strong></td>
      <td>Other languages — <strong>deferred</strong> at first; many are <strong>done</strong> in the tree now under <code>pb/</code></td>
    </tr>
  </tbody>
</table>
