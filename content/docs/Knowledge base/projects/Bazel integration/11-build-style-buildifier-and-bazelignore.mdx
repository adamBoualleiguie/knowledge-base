---
title: "Build style: Buildifier, headers, and `.bazelignore` speed"
description: "Starlark hygiene—Buildifier, license headers, and `.bazelignore` rules that keep analysis fast once the graph spans Go, Node, and CI noise."
...
# Build style: Buildifier, headers, and `.bazelignore` speed
...
<Terminal
  ...
  commands={[
    {
      command: "buildifier -r pb src/checkout docs",
      output: "",
    },
  ]}
/>
  ...
  commands={[
    {
      ...
      output: "# Broad package discovery (should complete in reasonable time on a clean tree)",
    },
    {
      command: "bazelisk query 'kind(\".*_library\", //...)' 2>/dev/null | wc -l",
      output: "# Optional: count packages (illustrative \u2014 output varies with graph)",
    },
  ]}
/>
